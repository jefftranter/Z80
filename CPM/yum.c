...
Up to 2 players (set at compile time, limited by screen size).
...
char *computerNames[MAXPLAYERS] = { "Altair", "IMSAI" };
...
void displayHelp()
{
    printf("%s",
           ...
           "\n"
           "This version supports up to two\n"
 ...
 * return true and 2.
 */
int possibleHighStraight(int *d)
{
    int i;
    int count = 0;
    ...
    while (1) {
        ...
        for (currentRound = 1; currentRound <= MAXROUNDS; currentRound++) {
            for (player = 0; player < numHumanPlayers + numComputerPlayers; player++) {
                ...
                clearScreen();
                sprintf(buffer, "%s's turn. Press <Return> to roll", playerName[player]);
                pressEnter(buffer);
                markAllDiceToBeRolled();
